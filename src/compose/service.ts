...
export class Service {
	...
	public isEqualConfig(
		...
		for (const [a, b] of [
			...
			if (aMatch != null) {
				if (bMatch != null) {
					// They're both service: <name> network modes
					...
					if (containerMatch == null) {
						...
						// We never want to switch sameNetworkMode back
				...
				// Only consider the hostname when it's not a
				// service: entry
				if (!sameNetworkMode) {
					sameNetworkMode =
						a === b && this.config.hostname === service.config.hostname;
				}
			}
		}
